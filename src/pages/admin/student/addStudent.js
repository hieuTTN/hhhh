...
const AdminAddPerson = ()=>{
    ...
    const [initClass, setClass] = useState(null);
    useEffect(()=>{
        ...
        const loadInit = async() =>{
            if(id != null){
                const response = await requestGet('http://localhost:8080/api/person/admin/findById?id='+id);
                var result = await response.json();
                setPerson(result)
                if(result.classes != null){
                    setClass({id:result.classes.id,name:result.classes.name})
                }
            }
        };
        loadInit();
    ...
    async function savePerson(event){
        ...
        const payload = { 
            person:{
                ...
                classes:{
                    id:event.target.elements.lophoc.value
                }
            },
            user:{
    ...
    return(
        <form onSubmit={savePerson} className='row' method='post'>
            ...
            <div className="col-sm-5">
                ...
                <label className="lb-form">Chọn lớp học</label>
                <Select
                value={initClass}
                onChange={(item) => {
                    setClass(item);
            ...
            <div className="col-sm-7">
                ...
                <button className='btn btn-primary form-control'>Thêm/ cập nhật sinh viên</button>
            </div>
        </form>
    );
}
